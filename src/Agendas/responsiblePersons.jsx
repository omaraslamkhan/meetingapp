...
export default function Participants(props) {
  ...
  React.useEffect(() => {
    const addedParticipants = props.currentTask.assignees;
    const addedIDs = [];
    const employeeIDs = [];
    const finalList = [];
  ...
  const addParticipants = () => {
    props.currentTask.assignees = addedParticipants;
    setCloseModal(false);
  };
    ...
    <div>
      <FormControl sx={{ m: 1, width: 600 }}>
        <div style={{ padding: 7 }}>
          ...
          >
            {!displayParticipants.length ? (
              <span style={{ marginRight: 5 }}>Add a Assignee(s)</span>
            ) : (
        ...
        <Autocomplete
          ...
          sx={{ width: "95%" }}
          renderInput={(params) => (
            <TextField {...params} label="Select Assignees" />
          )}
        />
